...
public:
  ...
  bool rectifyImage(cv::Mat& image, cv::Mat& image_rect, sensor_msgs::CameraInfo& camInfo)
  {
      cv::Mat mapx, mapy;
      cv::Mat K(3, 3, CV_64FC1, (void *) camInfo.K.data());
      cv::Mat D(1, 5, CV_64FC1, (void *) camInfo.D.data());
      cv::Mat R(3, 3, CV_64FC1, (void *) camInfo.R.data());
      cv::Mat P(3, 4, CV_64FC1, (void *) camInfo.P.data());
      try
      {
          cv::initUndistortRectifyMap(K, D, R, P, image.size(), CV_16F, mapx, mapy);
          cv::remap(image, image_rect, mapx, mapy, cv::INTER_CUBIC);
      }
      catch (cv::Exception& e)
      {
          ROS_ERROR("cv::undistort failed %s", e.what());
          return false;
      }
  }
...
public:
  ...
  ZedCameraROS()
  {
    ...
    while (nh.ok())
    {
      ...
      {
          if (zed.rectifyImage(left_image, left_image_rect, left_info))
          {
              publishImage(left_image_rect, left_image_rect_pub, "left_frame_rect", now);
          }
      }
      if (right_image_rect_pub.getNumSubscribers() > 0)
      {
          if (zed.rectifyImage(right_image, right_image_rect, right_info))
          {
              publishImage(right_image_rect, right_image_rect_pub, "right_frame_rect", now);
          }
      }
